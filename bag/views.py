...
def add_to_bag(request, item_id):
    ...
    if size:
        ...
        if item_id in list(bag.keys()):
            ...
            messages.success(request, f'One more {product.name} Added you got {bag[item_id]}')
        else:
            bag[item_id] = quantity
...
def adjust_bag(request, item_id):
    """ change the quantity of the specified product to the shopping bag """
    ...
    if size:
        ...
        if quantity > 0:
            bag[item_id] = quantity
            messages.success(request, f'Update {product.name} Added you got {bag[item_id]}')
...
def remove_from_bag(request, item_id):
    """ remove items from to the shopping bag """
    try:
        product = get_object_or_404(Product, pk=item_id)
        size = None
        if 'product_size' in request.POST:
